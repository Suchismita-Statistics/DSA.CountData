...
generated quantities{
  ...
  real s_full[full_len, 1];
  real density[full_len];  
  ...
  tau = algebra_solver(solution, tau_init, theta, x_r, x_i);
   ...
   s_full = integrate_ode_rk45(two_ode, ic, t0, full_time, parms, x_r, x_i);
   for(i in 1:full_len)
   {
     density[i] = b*s_full[i, 1]*(1+rho - s_full[i, 1] + g*log(s_full[i, 1])/b)/tau[1]; 
   }
}
...
full_time = seq(0.1, 108, 0.1)
full_len = length(full_time)
...
```{r}
ext      <- extract(fit)
den      <- ext$density        # matrix: n_draws x full_len

den_mean <- apply(den, 2, mean)
den_lo   <- apply(den, 2, quantile, probs = 0.025)
den_hi   <- apply(den, 2, quantile, probs = 0.975)

# ── Observed onset histogram ──────────────────────────────────────────────────
pl_list <- vector("list", 108)
for (i in 1:108)
  pl_list[[i]] <- runif(inf_ct[i], i - 1, i)

hist(unlist(pl_list),
     probability = TRUE,
     col         = "white",        # light grey looks cleaner than white
     border      = "black",
     breaks      = seq(0, 110, 10),
      main        = "Comparison of Estimated Density and Observed Counts for Ebola Data",
     xlab        = "Days",
     ylab        = "Density",
     ylim        = c(0, max(den_hi) * 1.1))
...
lines(full_time, den_mean, col = "red", lwd = 2)
```
