...
PDSIR_calc = function(data_Sellke,
                      rho,
                      Tmax = 10,
                      iter = 5e4,
                      b1 = 1,
                      g1 = 1)
{
  ...
  R1 = b1 / g1
  theta1 <- list(R0 = R1, # basic reproduction number
...
PDSIR_summary = function(result_list,
                         init_sus,
                         true_val,
                         Tmax = 10,
                         iter = 1e5,
                         ...
                         chain_num = 1)
{
  exPm = lapply(result_list, function(x)
    extract_param(x, chain_num, iter, warmup, init_sus))
  mn = apply(matrix(unlist(lapply(exPm, function(x)
    apply(x, 2, mean))), byrow = TRUE, ncol = 3), 2, mean)

  stddev = apply(matrix(unlist(lapply(exPm, function(x)
    apply(x, 2, sd))), byrow = TRUE, ncol = 3), 2, mean)
  temp1 = matrix(unlist(lapply(exPm, function(x)
    apply(x, 2, function(y)
      quantile(y, 0.025)))), byrow = TRUE, ncol = 3)
  temp2 = matrix(unlist(lapply(exPm, function(x)
    apply(x, 2, function(y)
      quantile(y, 0.975)))), byrow = TRUE, ncol = 3)

  R0per = mean(temp1[, 1] < true_val[1] & temp2[, 1] > true_val[1])
  gamma_per = mean(temp1[, 2] < true_val[2] &
                     temp2[, 2] > true_val[2])
  beta_per = mean(temp1[, 3] < true_val[3] &
                    temp2[, 3] > true_val[3])
  ...
  return(list(
    means = mn,
    sds = stddev,
    coverage = c(R0per, gamma_per, beta_per),
    ESS = esslist
  ))
}
...
for (i in 1:50)
{
  ...
  PDSIR_result[[i]] = PDSIR_calc(
    data_Sellke =  simulated_data[[i]],
    rho = 0.08,
    Tmax = 10,
    iter = 100,
    b1 = 0.1,
    g1 = 0.1
  ) ## rho is calculated so that we get around 20% acceptance probability.
}


temp = extract_param(
  result = PDSIR_result[[2]] ,
  chain_num = 1,
  iter = 1e5,
  warmup = 5e4,
  init_sus = 1e3,
